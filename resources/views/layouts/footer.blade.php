    ...
    <div class="container">
        <div class="footer-content">
            ...
            <div class="footer-section">
                ...
                <p>{{$setting['site-title'] ?? 'Phần mềm đánh giá dinh dưỡng'}}</p>
                <p style="margin-top: 15px;">{{$setting['site-description'] ?? 'Hệ thống đánh giá tình trạng dinh dưỡng cho trẻ em và người lớn theo tiêu chuẩn WHO'}}</p>
            </div>
            ...
            <div class="footer-section">
                ...
                <div class="footer-info-item">
                    ...
                    <span><a href="tel:{{$setting['phone']}}">{{$setting['phone'] ?? '0123456789'}}</a></span>
                </div>
                <div class="footer-info-item">
                    <i class="fas fa-envelope"></i>
                    <span><a href="mailto:{{$setting['email']}}">{{$setting['email'] ?? 'info@dinhduong.vn'}}</a></span>
                </div>
                @if(isset($setting['website']) && $setting['website'])
                <div class="footer-info-item">
        ...
        <div class="footer-bottom">
            <p>&copy; {{ date('Y') }} {{$setting['site-title'] ?? 'Phần mềm đánh giá dinh dưỡng'}}. All rights reserved.</p>
            <p>
                @if(isset($setting['app_version']) && $setting['app_version'])
                    Version {{$setting['app_version']}} | 
                @endif
                Developed with <i class="fas fa-heart" style="color: #e74c3c;"></i> 
                @if(isset($setting['developer']) && $setting['developer'])
                    by {{$setting['developer']}}
                @endif
            </p>
            
            <!-- Social Media Links (if configured) -->
            @if(isset($setting['facebook']) || isset($setting['zalo']) || isset($setting['youtube']))
            <div class="social-links">
                ...
                @endif
                @if(isset($setting['zalo']) && $setting['zalo'])
                    <a href="{{$setting['zalo']}}" target="_blank" title="Zalo">
