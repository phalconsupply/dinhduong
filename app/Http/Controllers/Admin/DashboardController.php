...
//        dd($dataRisk);
    ...
    private function getMeanStatistics($records)
    {
        ...
        foreach ($records as $record) {
            ...
            // Calculate z-scores
            $wa = $record->check_weight_for_age_auto();
            $ha = $record->check_height_for_age_auto();
            $wh = $record->check_weight_for_height_auto();
    ...
    public function getCellDetails(Request $request)
    {
        ...
        foreach ($records as $child) {
            ...
            switch ($category) {
                case 'weight_for_age':
                    $zscore = $child->getWeightForAgeZScoreAuto();
                    $zscoreType = 'W/A';
                    break;
                case 'height_for_age':
                    $zscore = $child->getHeightForAgeZScoreAuto();
                    $zscoreType = 'H/A';
                    break;
                case 'weight_for_height':
                    $zscore = $child->getWeightForHeightZScoreAuto();
                    $zscoreType = 'W/H';
                    break;
                case 'bmi_for_age':
                    $zscore = $child->getBMIForAgeZScoreAuto();
                    $zscoreType = 'BMI/A';
                    break;
                default:
