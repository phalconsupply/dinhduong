...
echo "=== IMPORT DỮ LIỆU BMI FOR AGE TỪ CSV ===\n\n";

// Xóa dữ liệu cũ
echo "Đang xóa dữ liệu cũ...\n";
Capsule::table('bmi_for_age')->truncate();
echo "✅ Đã xóa dữ liệu cũ\n\n";

/**
 * Hàm import dữ liệu BMI For Age từ file CSV với 2 nhóm tuổi
 */
function importBMIForAgeFromCSV($filePath, $gender, $genderName) {
    ...
    while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
        ...
        try {
            // Insert vào database
            Capsule::table('bmi_for_age')->insert([
                'gender' => $gender,
                'fromAge' => $fromAge,
                'toAge' => $toAge,
...
$boyFile = __DIR__ . '/zscore/BFA-Zscore - Boy.csv';
$boyCount = importBMIForAgeFromCSV($boyFile, 1, 'Nam');

// Import dữ liệu cho nữ (gender = 0) 
$girlFile = __DIR__ . '/zscore/BFA-Zscore - Girl.csv';
$girlCount = importBMIForAgeFromCSV($girlFile, 0, 'Nữ');

// Kiểm tra kết quả
$totalRecords = Capsule::table('bmi_for_age')->count();
...
$boyData = Capsule::table('bmi_for_age')
    ->where('gender', 1)
    ->orderBy('Months', 'asc')
    ->limit(5)
    ->get();
...
$girlData = Capsule::table('bmi_for_age')
    ->where('gender', 0)
    ->orderBy('Months', 'asc')
    ->limit(5)
    ->get();
...
echo "\n🎉 HOÀN THÀNH IMPORT DỮ LIỆU BMI FOR AGE!\n";
