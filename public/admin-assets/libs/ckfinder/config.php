...
//echo auth()->user()->hasAnyRole(['admin','manager','employee']);
$config['authentication'] = function () {
    if (auth()->guest()) {
        ...
        return false; // is_admin: attribute in model
    }
};
...
if(auth()->guest()){
    ...
    $folder = (auth()->user()->username == 'admin') ? 'app' : auth()->user()->username;
}
//echo $folder;
...
$config['resourceTypes'][] = array(
    'name'              => 'Ảnh của tôi',
    'directory'         => '/users/'.auth()->user()->username,
    'maxSize'           => 0,
